...
export default function Home() {
    ...
    const createNewProject = async () => {
        ...
        setIsCreating(true);
        try {
            // 1. ساختن پروسه جدید (اگر جدول processes دارید)
            ...
            // نکته: قبلاً اینجا کدی بود که ستون‌های پیش‌فرض (کارتابل و...) را می‌ساخت.
            // آن کد حذف شد تا پروژه خام باشد.

            setProjects([project, ...projects]);
            setNewProjectTitle("");
            toast.success("پروژه جدید ساخته شد");
        } catch (err: any) {
            toast.error("خطا در ساخت پروژه: " + err.message);
        } finally {
            setIsCreating(false);
        }
    };
    ...
    const deleteProject = async (id: number) => {
        if (!confirm("آیا از حذف این پروژه اطمینان دارید؟ تمام تسک‌ها پاک خواهند شد.")) return;
        try {
        ...
        <div className="p-6 md:p-10 text-white min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
                ...
                <div className="relative w-full md:w-64">
                    ...
                    <input className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pr-10 pl-4 text-sm focus:outline-none focus:border-blue-500/50 transition" placeholder="جستجو..." />
                </div>
            </div>
                ...
                <div className="glass p-1 rounded-3xl border border-dashed border-white/20 hover:border-blue-500/50 transition group cursor-pointer h-48 flex flex-col items-center justify-center relative">
                    {!isCreating ? (
                        <div onClick={() => setIsCreating(true)} className="flex flex-col items-center justify-center w-full h-full">
                            <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mb-3 group-hover:bg-blue-500 group-hover:text-white transition">
                                <FiPlus size={24} />
                            </div>
                        ...
                        <div className="p-4 w-full flex flex-col items-center gap-3 animate-fade-in">
                            <input 
                                autoFocus
                                className="w-full bg-black/40 border border-white/10 rounded-lg p-2 text-center text-sm text-white focus:border-blue-500 outline-none"
                    ...
                    <div key={project.id} className="glass p-6 rounded-3xl border border-white/5 hover:border-white/10 transition relative group h-48 flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                            ...
                            <button 
                                ...
                                className="text-white/20 hover:text-red-400 p-2 opacity-0 group-hover:opacity-100 transition"
                            >
                                <FiTrash2 />
                            </button>
                        </div>
